...
const ToolsGrid = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-background to-divine/20 p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {tools.map((tool, index) => {
            ...
            return (
              <div
                key={index}
                className="bg-card/50 backdrop-blur-sm border border-border rounded-lg p-6 hover-scale cursor-pointer transition-all duration-300 hover:border-divine/30"
                onClick={() => navigate(tool.path)}
              >
                <div className="flex items-center gap-3 mb-4">
                  <IconComponent className={`w-6 h-6 text-${tool.color}`} />
